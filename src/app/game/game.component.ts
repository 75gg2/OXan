...
export class GameComponent {
  ...
  checkPoints() {
    ...
    for (let i1 = 0; i1 < this.board.length; i1++) {
      ...
      for (let i = 0; i < row.length; i++) {
        ...
        if (row[i] == temp.player) temp.inRow++; else temp.inRow = 1
        temp.player = row[i]
    ...
    for (let i1 = 0; i1 < this.board[0].length; i1++) {
      ...
      for (let i = 0; i < column.length; i++) {
        ...
        if (column[i] == temp.player) temp.inRow++; else temp.inRow = 1
    ...
    for (let i1 = -1 * this.board.length; i1 < this.board[0].length; i1++) {
      ...
      for (let i = 0; i < skos.length; i++) {
        ...
        if (skos[i] == temp.player) temp.inRow++; else temp.inRow = 1
        temp.player = skos[i]
        if (temp.inRow == 5 && temp.player < 3) {
          if (temp.player == 1) this.points.o++
          if (temp.player == 2) this.points.x++
    ...
    if(this.points.o == this.gameProps.p){
      this.end.emit({message:`Gracz O wygrał ${this.points.o} do ${this.points.x}`})
    }else if(this.points.x == this.gameProps.p){
      this.end.emit({message:`Gracz X wygrał ${this.points.x} do ${this.points.o}`})
    }
    else if(left==0){
      if(this.points.x<this.points.o)this.end.emit({message:`Koniec pól. Gracz O wygrał ${this.points.o} do ${this.points.x}`}); else
      if(this.points.x>this.points.o)this.end.emit({message:`Koniec pól. Gracz X wygrał ${this.points.x} do ${this.points.o}`}); else
      this.end.emit({message:`Remis ${this.points.o} do ${this.points.x}`})
    }
  }
  ...
  ai() {
    ...
    for (let i1 = 0; i1 < this.board.length; i1++) {
      ...
      const temp = {player: 0, inRow: 0}
      for (let i = 0; i < row.length; i++) {
        if (row[i] == temp.player) temp.inRow++; else {
          if (temp.player == 1 || temp.player == 2) {
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i1, n: i}})
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i1, n: i - temp.inRow - 1}})
          }
          temp.inRow = 1
        }
        temp.player = row[i]
    ...
    for (let i1 = 0; i1 < this.board[0].length; i1++) {
      ...
      const temp = {player: 0, inRow: 0}
      for (let i = 0; i < column.length; i++) {
        if (column[i] == temp.player) temp.inRow++; else {
          if (temp.player == 1 || temp.player == 2) {
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i, n: i1}})
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i - temp.inRow - 1, n: i1}})
          }
          temp.inRow = 1
        }
    ...
    for (let i1 = -1 * this.board.length; i1 < this.board[0].length; i1++) {
      ...
      const temp = {player: 0, inRow: 0}
      for (let i = 0; i < skos.length; i++) {
        if (skos[i] == temp.player) temp.inRow++; else {

          if (temp.player == 1 || temp.player == 2) {
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i, n: i1 + i}})
            a.push({moc: temp.inRow, player: temp.player, pole: {m: i - temp.inRow -1, n: i1 + i - temp.inRow -1}})
          }
          temp.inRow = 1
        }
        temp.player = skos[i]
    ...
    for (let move of moves) {
      if(this.board[move.m]&&this.board[move.m][move.n]!==undefined&&this.board[move.m][move.n]==0) {
